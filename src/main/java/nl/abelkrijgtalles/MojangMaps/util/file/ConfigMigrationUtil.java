...
public class ConfigMigrationUtil {
    ...
    private static void migrateFrom2(FileConfiguration config, Logger logger) throws IOException {
        ...
        newConfig.append("# Config-version is auto-generated by Mojang Maps, so don't change this. When changed, undefined behavior can and will probably happen\n");
        newConfig.append("config-version: 3\n\n");
        newConfig.append("# If true, then the plugin shows the current street in the ActionBar (a box of text right above the name of items).\n");
        newConfig.append("street-actionbar: ");
        newConfig.append(streetActionbar);
        newConfig.append("\n\n");
        newConfig.append("# If you change this to custom, you can change the messages in messages.yml. Codes that can be used as language: https://github.com/Abelkrijgtalles/mojang-maps-data/blob/main/README.md#the-following-codes-can-be-used-as-language\n");
        newConfig.append("language: ");
        newConfig.append(language);

        config.save(newConfig.toString());
